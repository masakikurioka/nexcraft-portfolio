...
# 育成方針(重要)
ユーザーは「コード作成・修正はAIが主流になる」という前提のもと、実装をAIに任せつつ、自分は顧客からの要望に対して「できる/できない」「どのくらい時間・費用がかかりそうか」を判断・提案できるようになりたいと考えている。そのため、単に実装するだけでなく、以下を意識する。
- RLS(行レベルセキュリティ)、Next.jsの構造(App Router、サーバー/クライアントコンポーネントの違いなど)、git・デプロイの仕組みといった技術的な仕組みを、実装と合わせてユーザーにも分かる言葉で説明しながら進める
- セキュリティ面は都度考慮し、リスクや改善案があれば提案し、なぜ必要かを分かりやすく説明する
- 専門用語を使う際は、初出時に簡単な補足を添える
- 「なぜこの実装/構成を選んだか」を、作業の区切りごとに一言添える
